...
def view_student_info():
    ...
    while True:
        try:
            time.sleep(.5)
            student_id = input("Please enter the ID of the student whose information you would like to view.\n> ")
        except ValueError:
            time.sleep(.15)
    ...
    if results:                                 # the ifs check if there was data found in the results
        ...
        print("No results were found for that query.")      # prints out no data was found for query, no longer have to account for the total number of students
